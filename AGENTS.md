...
非自明な調査・実装・検証では、ユーザーが明示しなくても作業開始時に複数エージェントを起動して高速化する。
起動しない場合は、開始時に「なぜ起動しないか」を日本語で明示する。
...
- 非自明な調査・実装・レビュー・検証では、ユーザーが「複数エージェントで」と書いていなくても、作業開始直後に原則2つ以上のエージェントを起動する
- まず独立タスクを切り出してエージェントへ渡し、その後に親エージェントが自分の担当作業を進める
- 「あとで必要になったらspawnする」ではなく、最初に並行化できる論点を探す
- 並行化できる論点が本当にない場合だけspawnしない。その場合は、作業開始時に理由を明示する
- Handoffを作成する場合は、別Codexが見落とさないよう、英語で `Multi-Agent Requirement` セクションを入れる
- 最終報告には、使ったエージェントの担当、回収した結果、close済みかを含める。使わなかった場合は理由を含める
